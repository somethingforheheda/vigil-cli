...
import { TOPMOST_LEVEL_WIN } from "./constants/platform";
...
function linuxGetOpenAtLogin(): boolean {
  try { return fs.existsSync(AUTOSTART_FILE); } catch { return false; }
}

function linuxSetOpenAtLogin(enable: boolean): void {
  if (enable) {
    ...
    try {
      ...
      console.warn("VigilCLI: failed to write autostart entry:", (err as Error).message);
    }
  } else {
...
export function initMenu(ctx: MenuContext): {
  t(key: string): string;
  ...
  createTray(): void;
  destroyTray(): void;
  setShowTray(val: boolean): void;
  applyDockVisibility(): void;
  setShowDock(val: boolean): void;
  ensureContextMenuOwner(): BrowserWindow | null;
  popupMenuAt(menu: Menu): void;
  showContextMenu(): void;
  setLanguage(newLang: string): void;
  sendPrefsToRenderer(): void;
  requestAppQuit(): void;
} {
  // ── Translation helper ──
  function t(key: string): string {
    const lang = ctx.lang as keyof typeof i18n;
    const table = (i18n[lang] || i18n.en) as Record<string, string>;
  ...
  function setShowTray(val: boolean): void {
  ...
  function setShowDock(val: boolean): void {
    ...
    items.push(
      ...
      { type: "separator" },
      {
        ...
        submenu: [
          ...
          { label: t("themePure"), type: "radio", checked: ctx.theme === "pure", click: () => setTheme("pure") },
          { label: t("themeMint"), type: "radio", checked: ctx.theme === "mint", click: () => setTheme("mint") },
        ],
      },
      {
        label: t("textSize"),
  ...
  function ensureContextMenuOwner(): BrowserWindow | null {
  ...
  function popupMenuAt(menu: Menu): void {
      ...
      callback: () => {
        ...
        if (ctx.win && !ctx.win.isDestroyed()) {
          ...
          if (isMac) {
            ...
            ctx.win.setAlwaysOnTop(true, TOPMOST_LEVEL_WIN);
          }
        }
      },
    });
