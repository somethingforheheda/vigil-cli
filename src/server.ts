...
function syncVigilCLIHooks(): void {
  try {
    ...
    const { registerHooks } = require("../hooks/src/install.ts") as { registerHooks: (opts: object) => { added: number; updated: number; removed: number } };
...
function syncGeminiHooks(): void {
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { registerGeminiHooks } = require("../hooks/src/gemini-install.ts") as { registerGeminiHooks: (opts: object) => { added: number; updated: number } };
...
function syncCodeBuddyHooks(): void {
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { registerCodeBuddyHooks } = require("../hooks/src/codebuddy-install.ts") as { registerCodeBuddyHooks: (opts: object) => { added: number; updated: number } };
...
function syncCursorHooks(): void {
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { registerCursorHooks } = require("../hooks/src/cursor-install.ts") as { registerCursorHooks: (opts: object) => { added: number; updated: number } };
    ...
    if (req.method === "GET" && req.url === "/state") {
      ...
      req.on("end", () => {
        ...
        try {
          ...
          const { state, svg, session_id, event } = data as {
            state: unknown; svg: unknown; session_id: unknown; event: unknown;
          };
          let display_svg: string | null | undefined;
          ...
          const source_pid = Number.isFinite(data.source_pid) && (data.source_pid as number) > 0
          ...
          if (typeof state === "string" && ctx.validStates.has(state as import("./constants/states").AgentState)) {
