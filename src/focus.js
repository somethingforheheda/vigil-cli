"use strict";
// src/focus.ts — Terminal focus system (PowerShell persistent process + macOS osascript)
// Ported from vigil-cli/src/focus.js
...
function initFocus(ctx) {
    ...
    function initFocusHelper() {
        if (!isWin || psProc)
            return;
        psProc = (0, child_process_1.spawn)("powershell.exe", ["-NoProfile", "-NoLogo", "-NonInteractive", "-Command", "-"], {
            windowsHide: true,
    ...
    function flushQueuedMacFocus() {
        if (!macQueuedFocusRequest || macFocusInFlight)
            return;
    ...
    function requestMacFocus(sourcePid, cwd, editor, pidChain) {
        ...
        if (inCooldown && macFocusLastPid === sourcePid)
            return;
        ...
        if (isMac) {
            ...
            if (Array.isArray(pidChain)) {
                for (const pid of pidChain) {
                    if (!Number.isFinite(pid) || pid <= 0 || pidCandidates.includes(pid))
                        continue;
                    pidCandidates.push(pid);
                    if (pidCandidates.length >= 3)
                        break;
                }
            }
      ...
      tell application "System Events"
        repeat with targetPid in {${applePidList}}
          ...
          if (count of pList) > 0 then
            ...
            (0, child_process_1.execFile)("osascript", ["-e", script], { timeout: MAC_FOCUS_TIMEOUT_MS }, (err) => {
                if (err)
                    console.warn("focusTerminal macOS failed:", err.message);
                if (onDone)
                    onDone();
            });
            return;
        ...
        if (isLinux) {
            ...
            const tryXdoTool = () => {
                (0, child_process_1.execFile)("xdotool", ["search", "--pid", String(sourcePid), "windowactivate", "--sync"], {
                    timeout: 1200,
                }, () => {
                    if (onDone)
                        onDone();
            ...
            (0, child_process_1.execFile)("wmctrl", ["-lp"], { timeout: 1000 }, (err, stdout) => {
                if (err || !stdout)
                    return tryXdoTool();
                ...
                if (!winId)
                    return tryXdoTool();
                (0, child_process_1.execFile)("wmctrl", ["-i", "-a", winId], { timeout: 1000 }, (activateErr) => {
                    if (activateErr)
                        return tryXdoTool();
                    if (onDone)
                        onDone();
        ...
        if (cwd) {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                if (!name || name === dir || /^[A-Z]:$/i.test(name))
                    break;
        ...
        if (psProc && psProc.stdin && psProc.stdin.writable) {
            psProc.stdin.write(cmd + "\n");
        }
        else {
            // Fallback: one-shot PowerShell if persistent process died
            psProc = null;
            (0, child_process_1.execFile)("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", PS_FOCUS_ADDTYPE + cmd], { windowsHide: true, timeout: 5000 }, (err) => { if (err)
                console.warn("focusTerminal failed:", err.message); });
