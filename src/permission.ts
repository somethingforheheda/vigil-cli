...
import type { PermContext, PermissionEntry } from "./types/ctx";
import { TOPMOST_LEVEL_WIN, LINUX_WINDOW_TYPE } from "./constants/platform";
import { IpcChannels } from "./constants/ipc-channels";
import { VIGILCLI_SERVER_HEADER, VIGILCLI_SERVER_ID } from "../hooks/dist/server-config";
import * as path from "path";

// eslint-disable-next-line @typescript-eslint/no-require-imports
const { BrowserWindow: ElectronBrowserWindow, globalShortcut } = require("electron") as typeof import("electron");
...
export function initPermission(ctx: PermContext): {
  ...
  PASSTHROUGH_TOOLS: ReadonlySet<string>;
  handleBubbleHeight(event: IpcMainEvent, height: number): void;
  handleDecide(event: IpcMainEvent, behavior: string): void;
  cleanup(): void;
  showCodexNotifyBubble(opts: { sessionId: string; command?: string }): void;
  clearCodexNotifyBubbles(sessionId: string): void;
  syncPermissionShortcuts(): void;
} {
  ...
  const measuredBubbleWidths = new WeakMap<PermissionEntry, number>();
  ...
  function syncPermissionShortcuts(): void {
    ...
    if (shouldRegister && !hotkeysRegistered) {
      ...
      try { globalShortcut.unregister(HOTKEY_ALLOW); } catch { /* ignore */ }
  ...
  function estimateBubbleHeight(sugCount: number): number {
    return 200 + (sugCount || 0) * 37;
  }

  // ── Bubble move animation ──
  const bubbleAnimations = new WeakMap<BrowserWindow, { timer: ReturnType<typeof setInterval> }>();

  function animateBubbleTo(win: BrowserWindow, targetX: number, targetY: number, w: number, h: number): void {
  ...
  function stackBubbles(): void {
    ...
    if (ctx.bubbleFollowWindow) {
      ...
      const sessionYCursors = new Map<string, number>();
  ...
  function destroyBubbleEntry(entry: PermissionEntry): void {
    const bub = entry.bubble;
    if (bub && !bub.isDestroyed()) {
      bub.webContents.send(IpcChannels.PERMISSION_HIDE);
  ...
  function showPermissionBubble(permEntry: PermissionEntry): void {
    ...
    let bubbleSide: "left" | "right" | null = null;
    ...
    const bub: BrowserWindow = new ElectronBrowserWindow({
      width: pos.width,
      height: pos.height,
      x: pos.x,
      y: pos.y,
    ...
    if (isWin) {
      bub.setAlwaysOnTop(true, TOPMOST_LEVEL_WIN);
    }

    bub.webContents.once("did-finish-load", () => {
      bub.webContents.send(IpcChannels.PERMISSION_SHOW, {
  ...
  function resolvePermissionEntry(permEntry: PermissionEntry, behavior: string, message?: string): void {
    ...
    const decision: { behavior: string; message?: string; updatedPermissions?: unknown[] } = {
  ...
  function permLog(msg: string): void {
    ...
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const { rotatedAppend } = require("./log-rotate") as { rotatedAppend: (file: string, data: string) => void };
  ...
  function sendPermissionResponse(
    ...
    let decision: { behavior: string; message?: string; updatedPermissions?: unknown[] };
  ...
  function handleBubbleHeight(event: IpcMainEvent, size: number | { width?: number; height?: number }): void {
  ...
  function handleDecide(event: IpcMainEvent, behavior: string): void {
  ...
  function showCodexNotifyBubble({ sessionId, command }: { sessionId: string; command?: string }): void {
    ...
    const permEntry: PermissionEntry = {
      res: null,
      abortHandler: null,
      suggestions: [],
      sessionId,
  ...
  function dismissCodexNotify(permEntry: PermissionEntry): void {
    const idx = pendingPermissions.indexOf(permEntry);
    if (idx === -1) return;
    pendingPermissions.splice(idx, 1);
    ...
    if (permEntry.bubble && !permEntry.bubble.isDestroyed()) {
      permEntry.bubble.webContents.send(IpcChannels.PERMISSION_HIDE);
  ...
  function clearCodexNotifyBubbles(sessionId: string): void {
