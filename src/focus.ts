...
interface FocusContext {
  _allowSetForeground: ((pid: number) => boolean) | null;
}
...
export function initFocus(ctx: FocusContext): {
  initFocusHelper(): void;
  killFocusHelper(): void;
  focusTerminalWindow(
    sourcePid: number,
    cwd: string | null | undefined,
    editor: string | null | undefined,
    pidChain: number[] | null | undefined,
  ): void;
  clearMacFocusCooldownTimer(): void;
  cleanup(): void;
} {
  // Persistent PowerShell process — warm at startup, reused for all focus calls
  let psProc: ReturnType<typeof spawn> | null = null;
  ...
  function killFocusHelper(): void {
    if (psProc) { psProc.kill(); psProc = null; }
  }

  function scheduleTerminalTabFocus(
    editor: string | null | undefined,
    pidChain: number[] | null | undefined,
  ): void {
  ...
  function clearMacFocusCooldownTimer(): void {
  ...
  function scheduleQueuedMacFocus(delayMs: number): void {
  ...
  function flushQueuedMacFocus(): void {
  ...
  function focusTerminalWindow(
    ...
    if (ctx._allowSetForeground && psProc && psProc.pid) {
      try { ctx._allowSetForeground(psProc.pid); } catch { /* ignore */ }
    }
