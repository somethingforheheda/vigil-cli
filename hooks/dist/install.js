...
function registerHooks(options = {}) {
  ...
  let hookScript = path2.resolve(__dirname, "..", "dist", "watchcli-hook.js").replace(/\\/g, "/");
  ...
  for (const event of hookEvents) {
    ...
    const desiredCommand = options.remote ? `CLAWD_REMOTE=1 "${nodeBin}" "${hookScript}" ${event}` : `"${nodeBin}" "${hookScript}" ${event}`;
