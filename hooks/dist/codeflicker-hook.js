...
var argState = argEvent ? EVENT_TO_STATE[argEvent] : void 0;
...
process.stdin.on("end", () => {
  ...
  try {
    ...
    const subagentId = event === "SubagentStart" || event === "SubagentStop" ? String(payload.agent_id ?? "") : "";
...
function send(sessionId, cwd, sessionTitle, event, state, subagentId, extras) {
  if (sent) return;
  sent = true;
