...
function displaySvgFromToolHook(hookName, payload) {
  if (hookName !== "preToolUse" && hookName !== "postToolUse") return void 0;
...
function runWithPayload(payload) {
  ...
  if (hookNameResolved === "sessionStart" && !process.env.VIGILCLI_REMOTE) findTerminalPid();
  const sessionId = String(payload?.conversation_id ?? payload?.session_id ?? "default");
  let cwd = String(payload?.cwd ?? "");
  if (!cwd && Array.isArray(payload?.workspace_roots) && payload.workspace_roots[0]) {
  ...
  if (hint !== void 0) body.display_svg = hint;
  if (cwd) body.cwd = cwd;
